...
class ContextDecoder2D(nn.Module):
    def __init__(
        self,
        state_encoded_dim,
        action_emb_dim,
        ...
        # --- New projection and attention layers ---
        proj_dim = 1024
        # Project the state and action to a 1024-dimensional space.
        self.state_fc_1024 = nn.Linear(state_encoded_dim, proj_dim).to(DEVICE)
        self.action_fc_1024 = nn.Linear(action_emb_dim, proj_dim).to(DEVICE)
        # Perform attention: let the state (query) attend to the action (key and value).
        self.attn = nn.MultiheadAttention(embed_dim=proj_dim, num_heads=num_heads, batch_first=True).to(DEVICE)
        # Project back to the original embedding dimension.
        self.fc_back_state = nn.Linear(proj_dim, emb_dim).to(DEVICE)
        self.fc_back_action = nn.Linear(proj_dim, emb_dim).to(DEVICE)
    ...
    def forward(self, x_t, e_t, tgt_key_padding_mask=None):
        ...
        # Project state and action to 1024 dimensions.
        x_1024 = self.state_fc_1024(x_t)   # [B, 1024]
        e_1024 = self.action_fc_1024(e_t)    # [B, 1024]
        # Reshape to add the sequence dimension (required by MultiheadAttention).
        x_1024_unsq = x_1024.unsqueeze(1)    # [B, 1, 1024]
        e_1024_unsq = e_1024.unsqueeze(1)    # [B, 1, 1024]
        # Let the state (query) attend to the action (key and value).
        attn_output, _ = self.attn(query=x_1024_unsq, key=e_1024_unsq, value=e_1024_unsq)
        attn_output = attn_output.squeeze(1)  # [B, 1024]

        # Project back to the embedding dimension and add context positional embeddings.
        x_proj = self.fc_back_state(x_1024) + self.ctx_pos_emb[0]   # [B, emb_dim]
        e_proj = self.fc_back_action(attn_output) + self.ctx_pos_emb[1]  # [B, emb_dim]
        # Stack to create memory tokens.
        memory = torch.stack([x_proj, e_proj], dim=1)    # [B, 2, emb_dim]
        memory = self.memory_layer_norm(memory)

        # --- Prepare target tokens (same as before) ---
        # For the first two tokens, add 1D positional embeddings.
        first_two = self.query_emb[:2] + self.pos_emb         # [2, emb_dim]
        first_two = self.shape_token_transform(first_two)       # Enhance representation
        # For the grid tokens, add 2D positional embeddings.
