...
@nb.njit
def select_color_numba(grid, method, param):
    """
    Select a color based on the given method and parameter.
    (Note: In numba nopython mode strings are handled as bytes.)
    """
    if method == b'color_rank':
        return rankcolor_numba(grid, param)
    elif method == b'shape_rank_nodiag':
        return rank_largest_shape_color_nodiag_numba(grid, param)
    elif method == b'shape_rank_diag':
...
class TransformerNumba:
    ...
    def color(self, grid, selection, method, param):
        ...
        """
        c = select_color_numba(grid, method, param)
        grid_3d = create_grid3d_numba(grid, selection)
        n_rows = grid.shape[0]
        n_cols = grid.shape[1]
        for i in range(n_rows):
    ...
    def fill_with_color(self, grid, selection, method, param):
        ...
        # Simple check for valid color (assume valid if 0<=color<10)
        if not (fill_color >= 0 and fill_color < 10):
            return grid_3d
        n_rows = grid.shape[0]
        n_cols = grid.shape[1]
        # Assume selection is 2D for simplicity.
    ...
    def fill_bounding_rectangle_with_color(self, grid, selection, method, param):
        ...
        for i in range(n_rows):
            for j in range(n_cols):
                # In our simplified version, if bound[i,j] is True and selection is False then fill.
    ...
    def rotate(self, grid, selection, num_rotations):
        ...
        if max_row >= min_row and max_col >= min_col and ((max_row - min_row) == (max_col - min_col)):
            ...
            # Perform num_rotations 90° rotations
    ...
    # (Other methods would be translated in a similar fashion.)
...
def main():
    # Create a 30x30 random grid with colors 0..9
    grid = np.random.randint(0, 10, size=(30, 30)).astype(np.int64)
    # Create a simple selection mask (for example, cells equal to 3)
    selection = (grid == 3)
